...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Query(value = "SELECT * FROM report_table WHERE user_id.id = :userId", nativeQuery = true)
    List<Report> findAllReportsFromUserId(@Param("userId") Long userId);

    Report findReportByDate(LocalDate currentDate);

    @Query(value = "DELETE * FROM report_table WHERE user_id.id = :userId", nativeQuery = true)
